'''
This file reads the csv file in the maestro dataset and creates a hdf5 file.
'''
...
def MakeHDF5(hdf5_path):
    #get path to the dataset

    maestro_dataset_path = os.path.abspath(os.path.join(os.path.dirname(__file__),"..", 'dataset'))
    maestro_CSV_path = os.path.join(maestro_dataset_path,'maestro-v3.0.0', 'maestro-v3.0.0.csv')
    #print(os.path.join(os.path.join(os.path.dirname(__file__),'maestro-v3.0.0-hdf5.hdf5')))
    #hdf5_path = os.path.join(os.path.join(os.path.dirname(__file__),'maestro-v3.0.0-hdf5.hdf5'))
    ...
    with h5py.File(hdf5_path, 'w') as f:
        for file in tqdm.tqdm(range(1,csv_lines_count)):
            ...
            midi_group = f.create_group(only_midi_name)
            ...
            #print(group)
            midi_group.attrs.create('midi_filename', data=midi_path)
            midi_group.attrs.create('audio_filename', data=audio_path)
            midi_group.attrs.create('composer', data=csv_line_split[0], dtype=h5py.string_dtype())
            #print(f"Storing composer: {csv_line_split[0]}")
            midi_group.attrs.create('title', data=csv_line_split[1], dtype=h5py.string_dtype())
            midi_group.attrs.create('split', data=csv_line_split[2], dtype=h5py.string_dtype())
            midi_group.attrs.create('year', data=csv_line_split[3], dtype=h5py.string_dtype())
            midi_group.attrs.create('duration', data=csv_line_split[6], dtype=h5py.string_dtype())
